...
public func appAssert(_ condition: @autoclosure () -> Bool, _ message: @autoclosure () -> String, file: StaticString = #file, function: StaticString = #function, line: UInt = #line) {
    if !condition() {
        appAssertionFailure(message(), file: file, function: function, line: line)
    }
}

public func appAssertionFailure(_ message: @autoclosure () -> String, file: StaticString = #file, function: StaticString = #function, line: UInt = #line) {
    let output: String
    let fileName: String
    ...
    let error = NSError(domain: "Assertion: " + message(), code: 1, userInfo: ["filename": fileName, "function": function, "line": line, "message": message(), "output": output])
